...
int main() {
    ...
    dur = (double)(end - start);
    printf("merge Sort Use Time:%f\n",(dur/CLOCKS_PER_SEC));

	start = clock();
	
	for(exetime = TEST_ROUND; exetime > 0; exetime--){
		generateRandomSequence(LENGTH, buffer);
		sort_heap2(buffer, LENGTH);
	}
	...
	memcpy((void *)buffer, (void *)pSequence, numOfByte);
	condition = selOperation(ASCENDING); 
	insertionSort(buffer, LENGTH, condition);
	print_array(buffer, LENGTH);

	memcpy((void *)buffer, (void *)pSequence, numOfByte);
	condition = selOperation(ASCENDING); 
	bubbleSort(buffer, LENGTH, condition);
	print_array(buffer, LENGTH);
#endif
...
#if 1 //Merge Sort itervation version
